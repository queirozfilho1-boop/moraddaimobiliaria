...
const ContratoEditorPage = () => {
  ...
  useEffect(() => {
    ...
    setContrato((c) => {
      if ((c.tipo === 'captacao_exclusiva' || c.tipo === 'autorizacao_venda' || c.tipo === 'autorizacao_padrao') && (c.comissao_venda_pct == null || c.comissao_venda_pct === 0)) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const papelProprietario: PartePapel | null =
        ...
        contrato.tipo === 'captacao_exclusiva' || contrato.tipo === 'autorizacao_venda' || contrato.tipo === 'autorizacao_locacao' || contrato.tipo === 'autorizacao_padrao' || contrato.tipo === 'administracao' ? 'proprietario' :
        null
  ...
  async function handleSave(novoStatus?: ContratoStatus) {
    ...
    const partesObrigatorias: Record<string, PartePapel[]> = {
      ...
      autorizacao_locacao: ['proprietario', 'imobiliaria'],
      autorizacao_padrao:  ['proprietario', 'imobiliaria'],
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      {contrato.tipo && usaVigencia(contrato.tipo) && (
        <Section icon={<Calendar size={16} />} title={
          ...
          contrato.tipo === 'autorizacao_venda' || contrato.tipo === 'autorizacao_locacao' || contrato.tipo === 'autorizacao_padrao' ? 'Vigência da Autorização' :
          'Vigência'
        }>
