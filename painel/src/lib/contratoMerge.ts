...
function buildContexto(args: MergeArgs): Record<string, any> {
  ...
  return {
    ...
    imovel: {
      ...
      quartos: imovel?.quartos != null ? `${imovel.quartos} ${imovel.quartos === 1 ? 'quarto' : 'quartos'}` : '',
      suites: imovel?.suites != null ? `${imovel.suites} ${imovel.suites === 1 ? 'suíte' : 'suítes'}` : '',
      banheiros: imovel?.banheiros != null ? `${imovel.banheiros} ${imovel.banheiros === 1 ? 'banheiro' : 'banheiros'}` : '',
      vagas: imovel?.vagas_garagem != null ? `${imovel.vagas_garagem} ${imovel.vagas_garagem === 1 ? 'vaga' : 'vagas'}` : '',
      vagas_garagem: imovel?.vagas_garagem != null ? `${imovel.vagas_garagem} ${imovel.vagas_garagem === 1 ? 'vaga de garagem' : 'vagas de garagem'}` : '',
      // Versões numéricas (quando o template já escreve "X quartos" hardcoded)
      quartos_num: imovel?.quartos ?? 0,
      suites_num: imovel?.suites ?? 0,
      banheiros_num: imovel?.banheiros ?? 0,
      vagas_num: imovel?.vagas_garagem ?? 0,
